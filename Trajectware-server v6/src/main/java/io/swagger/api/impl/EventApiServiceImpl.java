package io.swagger.api.impl;import java.sql.Connection;
...
public class EventApiServiceImpl extends EventApiService {
    ...
    public Response findEventsByInvention(  @NotNull List<String> invention, SecurityContext securityContext) throws NotFoundException {
    	...
    	String query = "(SELECT * FROM Event JOIN Participation ON EventId.Event = EventId.Participation";
    	Event eve = new Event();
    	// connection to the DB
    	Connection conn = DBManager.getConnection();
    	try (PreparedStatement preparedStmt = conn.prepareStatement(query)){
    ...
    @Override
    public Response findEventsByInventor(@NotNull List<String> invention, SecurityContext securityContext) throws NotFoundException {
    	// no need to verify that the List<String> is not null, because we have it verify in the parameters of the method 
