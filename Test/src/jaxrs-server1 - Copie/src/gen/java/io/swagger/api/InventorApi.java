...
,@Context SecurityContext securityContext)
    ...
    @io.swagger.annotations.ApiResponses(value = { 
        ...
        @io.swagger.annotations.ApiResponse(code = 400, message = "Invalid status value", response = Void.class) })
    public Response findInventorsByStatus(@ApiParam(value = "the status values that we need to considere to filter (is the inventor dead or alive)",required=true, allowableValues="alive, dead") @QueryParam("status") List<String> status
,@Context SecurityContext securityContext)
    throws NotFoundException {
...
,@Context SecurityContext securityContext)
    ...
    @io.swagger.annotations.ApiOperation(value = "find inventors by date", notes = "", response = Object.class, authorizations = {
